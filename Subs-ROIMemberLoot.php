...
// These constants really should be moved to their own class in a seperate file
...
function initializeClient() {
	...
	$client->setAssertionCredentials ( new Google_AssertionCredentials ( SERVICE_ACCOUNT_NAME, array (
...
// Adds the View Loot link into the global menu buttons array
...
function ROIMemberLoot_view_loot_action(&$actionArray) {
	$actionArray ['view_loot'] = array (
			'Subs-ROIMemberLoot.php',
			'ROIMemberLoot_view_loot' 
	);
}
...
function ROIMemberLoot_view_loot() {
	...
	loadTemplate ( 'ROIMemberLoot', array (
			'ROIMemberLoot' 
	) );
	...
	$arrContent = explode ( "\n", querySheet ( ROSTER_ID, "select%20*%20where%20lower(E)%20%3D%20'" . strtolower ( $user_info ['username'] ) . "'" ) );
	...
	if (count ( $arrContent ) == 3) {
		...
		if ($content) {
			...
			if (count ( $arrContent ) > 1) {
				...
				for($i = 0; $i < count ( $tiers ); $i ++) {
					...
					if ($tmp [0]) {
						$tierCounts [$tmp [0]] [COL_TIER_TOTAL] = 0;
						$tierCounts [$tmp [0]] [COL_TIER_VISIBLE] = 0;
						$tierCounts [$tmp [0]] [COL_TIER_NON_VIS] = 0;
						$tierCounts [$tmp [0]] [COL_TIER_WEAPON] = 0;
						$tierCounts [$tmp [0]] [COL_TIER_ROT] = 0;
						$tierCounts [$tmp [0]] [COL_TIER_ALT] = 0;
						$tierCounts [$tmp [0]] [COL_TIER_ALT_LL] = "";
						$tierCounts [$tmp [0]] [COL_TIER_LL] = "";
					}
				}
				...
				for($i = 1; $i < count ( $arrContent ); $i ++) {
					if ($arrContent [$i]) {
						...
						if (! $tmp [COL_IS_ROT] && ! $tmp [COL_IS_ALT]) {
							...
							if (strtotime ( $tierCounts [$lootTier] [COL_TIER_LL] ) < $tmp [COL_LOOT_TIME]) {
								$tierCounts [$lootTier] [COL_TIER_LL] = $tmp [COL_LOOT_DATE];
							}
							
							// Increment our total loot count
							$tierCounts [$lootTier] [COL_TIER_TOTAL] ++;
							
							// Check for visible counts
							if (in_array ( $tmp [COL_SLOT], $VISIBLE_LIST )) {
								$tierCounts [$lootTier] [COL_TIER_VISIBLE] ++;
							}
							
							// Check for non-visible counts
							if (in_array ( $tmp [COL_SLOT], $NON_VIS_LIST )) {
								$tierCounts [$lootTier] [COL_TIER_NON_VIS] ++;
							}
							
							// Check for weapon counts
							if (in_array ( $tmp [COL_SLOT], $WEAPON_LIST )) {
								$tierCounts [$lootTier] [COL_TIER_WEAPON] ++;
							}
						}
						
						// If this is a rot loot increment the rot loot count
						if ($tmp [COL_IS_ROT]) {
							$tierCounts [$lootTier] [COL_TIER_ROT] ++;
						}
						
						// If this is an alt loot increment the alt loot count
						if ($tmp [COL_IS_ALT]) {
							$tierCounts [$lootTier] [COL_TIER_ALT] ++;
							
							// Figure out the alt last loot date here
							if (strtotime ( $tierCounts [$lootTier] [COL_TIER_ALT_LL] ) < $tmp [COL_LOOT_TIME]) {
								$tierCounts [$lootTier] [COL_TIER_ALT_LL] = $tmp [COL_LOOT_DATE];
							}
						}
		...
		// Update the rot counts to be total rots minus total alts
		foreach ( $tierCounts as $key => $value ) {
			$tierCounts [$key] [COL_TIER_ROT] = $tierCounts [$key] [COL_TIER_ROT] - $tierCounts [$key] [COL_TIER_ALT];
		}
	...
	$context ['html_headers'] .= "\n<script type=\"text/javascript\" src=\"" . $settings ['default_theme_url'] . "/scripts/" . "jquery-2.1.1.min.js" . "\"></script>\n";
